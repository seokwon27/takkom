...
const LoadingHospitalList = ({ children, className, animate }: { children?: ReactNode; className?: string, animate?: string }) => {
  return (
    <div className={cn("flex flex-col items-center mt-4 mb-16 text-title-xl", className)}>
      {/* {children ? ( */}
        <>
          <Image src={HospitalLoading} alt="로딩중입니다." className={cn("max-sm:w-[160px] max-sm:aspect-sqare", animate)}/>
